...
class Maze(object):
    def __init__(self, filename):
        ...
        elif filename.lower().endswith('.maz'):
            with open(filename, 'rb') as f_in:
                buffer = f_in.read()
                # First line should be an integer with the maze dimensions
                self.dim = int(len(buffer) ** 0.5)

                # Subsequent lines describe the permissability of walls
                walls = []
                for i in range(0,len(buffer),self.dim):
                    # official mazes denote 1 as closed instead of open so we need to substract the output from 15
                    walls.append([15 - int(item) for item in buffer[i:i+self.dim]])  
                    
        else:
            raise FileNotFoundError(f'Unknown file extension for file {filename}')
